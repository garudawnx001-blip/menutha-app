...
export function Billing() {
  ...
  const gst = Math.round((subtotal - disc) * 5) / 100;
  ...
  const settle = async (mode: 'cash' | 'upi_qr') => {
    ...
    try {
      ...
      setBill(null); setSelected(new Set()); setDiscount('');
      await load();
    ...
    <div className="fade-in">
      ...
      {chosen.length > 0 && !bill && (
        <div className="glass-strong" style={{ padding: 16, marginTop: 16 }}>
          ...
          <div className="bill-row"><span>GST (5%)</span><span>{inr(gst)}</span></div>
        ...
        <div className="printable" style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontSize: 13 }}>
          ...
          <table style={{ width: '100%', marginTop: 8, borderCollapse: 'collapse' }}>
            <tbody>
              ...
              <tr><td>CGST 2.5%</td><td style={{ textAlign: 'right' }}>{inr(bill.gst_amount / 2)}</td></tr>
              <tr><td>SGST 2.5%</td><td style={{ textAlign: 'right' }}>{inr(bill.gst_amount / 2)}</td></tr>
